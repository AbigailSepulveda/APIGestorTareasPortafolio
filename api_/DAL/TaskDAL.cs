...
namespace api_.DAL {
    public class TaskDAL {
        ...
        public static decimal createTask(tasks tasks) {
            using (var conn = new db_entities()) {
                try {
                    conn.SP_TASK_INSERT(tasks.name, tasks.description, tasks.process_id, tasks.father_taks_id, tasks.task_status, DateTime.Now, tasks.creator_user_id, tasks.assing_id);
